...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(HH):
                for j in range(WW):
                    temp = x[n, c, i*stride:i*stride+ph, j*stride:j*stride+pw]
                    y = np.unravel_index(temp.argmax(), temp.shape) 
                    dx[n, c, i*stride:i*stride+ph, j*stride:j*stride+pw][y] = dout[n,c,i,j]
